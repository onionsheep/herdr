...
| Remote clipboard images, single files, and file drops | supported |
...
During `herdr --remote`, the configured remote image paste key checks for one regular file in the Windows clipboard before checking for an image, then transfers the accepted content to the remote host. Dropping one local regular file into Windows Terminal also transfers the file and pastes its remote path. Files must be non-empty and no larger than 16 MiB; directories and multiple selections are not transferred. The remote temporary copy is deleted when the client disconnects, so use or copy it before detaching.
...
Some Windows agents can receive `ctrl+v` and read clipboard images directly. Herdr's own clipboard file/image reader is not wired into local native Windows panes, so agent-native paste remains dependent on the terminal and agent. Agent image-paste shortcuts such as `alt+v` do not add a Herdr-managed local clipboard bridge. Remote clipboard file and image bridging is supported separately through `herdr --remote` to Linux, macOS, and Windows hosts.
...
| Herdr clipboard file/image bridge in local native panes | unsupported |
