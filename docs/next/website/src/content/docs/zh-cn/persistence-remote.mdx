...
这种模式下，远程服务器保持窗格运行，并通过 SSH 发送终端内容和会话状态。本地 Herdr 负责绘制 UI，包括侧边栏、菜单和主题。因为客户端在本地运行,Herdr 可以把剪贴板中的图像或一个普通文件桥接到远程会话: 把内容复制到远程临时文件,再粘贴该路径。把一个本地普通文件拖入外层终端时,也会使用同一套桥接。

配置的远程图像粘贴按键默认为 `ctrl+v`,触发时会先检查剪贴板文件,再检查剪贴板图像。每次操作只接受一个不超过 16 MiB 的非空普通文件;目录、多文件选择和更大的文件不会被传输。远程副本会保留经过安全清洗的原始文件 basename。它是连接级临时文件,不是持久上传: 客户端断开时 Herdr 会将其删除,同时也会清理超过 24 小时的崩溃遗留文件。请在分离前读取或复制该远程文件。
