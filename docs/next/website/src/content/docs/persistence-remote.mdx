...
In this mode, the remote server owns the running panes and sends their terminal content and session state over SSH. Your local Herdr draws the UI, including its sidebar, menus, and theme. Because the client runs locally, Herdr can bridge a clipboard image or one regular file into the remote session by copying the content to a remote temporary file and pasting that path. Dropping one local regular file into the outer terminal uses the same bridge.

The configured remote image paste binding, `ctrl+v` by default, checks for a clipboard file before it checks for a clipboard image. Each operation accepts exactly one non-empty regular file up to 16 MiB; directories, multiple selections, and larger files are not transferred. The remote copy keeps a safely normalized version of the original basename. It is connection-scoped rather than a persistent upload: Herdr removes it when that client disconnects, and also cleans up crash leftovers older than 24 hours. Read or copy the remote file before detaching.
